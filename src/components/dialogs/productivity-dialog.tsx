...
import {
  useIsOpen,
  useSetIsOpen,
} from "@/contexts/productivity-dialog-context";
import clsx from "clsx";
import { Suspense } from "react";

export default function ProductivityDialog() {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <Button
        type="button"
        aria-label="Open/close productivity dialog"
        className="hidden"
      />
      <Portal className="flex h-full items-start justify-center p-8 pt-[13vh]">
        <Panel
          className={clsx(
            "flex max-h-full w-full max-w-[450px] flex-col overflow-hidden rounded-[10px]",
