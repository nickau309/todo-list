...
import {
  useIsOpen,
  useSetIsOpen,
} from "@/contexts/upgrade-to-pro-dialog-context";
...
export default function UpgradeToProDialog() {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <Button
        type="button"
        aria-label="Open/close upgrade to pro dialog"
        className="hidden"
      />
      <Portal className="flex h-full items-start justify-center p-8 pt-[13vh]">
        <Panel
          className={clsx(
            "flex h-full max-h-[74vh] w-full max-w-3xl flex-col overflow-hidden rounded-[10px]",
        ...
        >
          <header className="flex items-center justify-between gap-4 p-2 pl-4">
            <Title className="text-lg/[23.2px] font-bold">Upgrade To Pro</Title>
            <Dismiss
              type="button"
              aria-disabled="false"
