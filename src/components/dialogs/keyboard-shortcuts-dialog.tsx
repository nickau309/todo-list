...
import {
  useIsOpen,
  useSetIsOpen,
} from "@/contexts/keyboard-shortcuts-dialog-context";
...
export default function KeyboardShortcutsDialog() {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <Button
        type="button"
        aria-label="Open/close keyboard shortcuts dialog"
        className="hidden"
      />
      <Portal className="flex h-full justify-end p-8 max-[400px]:p-0 max-[400px]:pt-8">
        <Panel
          className={clsx(
            "flex w-full max-w-[400px] flex-col overflow-hidden rounded-[10px]",
            "bg-background-base-primary text-display-primary-idle-tint",
            "shadow-[0_2px_8px_rgba(0,0,0,.16)]",
            "max-[400px]:rounded-b-none",
          )}
        >
          <header className="flex items-center justify-between gap-4 p-2 pl-4">
            <Title className="truncate text-base/5 font-bold">
              Keyboard Shortcuts
            </Title>
            <Dismiss
              type="button"
